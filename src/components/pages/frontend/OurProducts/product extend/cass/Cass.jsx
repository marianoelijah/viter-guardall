...
const ProductCard = ({ title, description, image, detailRoute }) => (
  <div className="bg-white border border-gray-200 rounded-xl p-5 flex flex-col h-full transition-all duration-300 ease-in-out hover:-translate-y-2 hover:shadow-xl cursor-pointer group">
    ...
    <div className="flex-grow">
      {/* Increased min-h for longer Parking System titles */}
      <h3 className="text-[#1a365d] font-semibold text-xl uppercase mb-3 leading-tight tracking-tight min-h-[4.5rem]">
        {title}
      </h3>
...
const Cass = () => {
  ...
  useEffect(() => {
    const fetchCassProducts = async () => {
      try {
        const response = await fetch(`${IMAGE_BASE_URL}/api/products/brand/Cass`);
        const data = await response.json();
        setProducts(data);
        setLoading(false);
      } catch (error) {
        console.error("Error fetching CASS products:", error);
        setLoading(false);
      }
    };
    fetchCassProducts();
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-figtree text-[#0d3874]">Loading Parking Solutions...</div>;
      ...
      <div className="max-w-7xl mx-auto">
        <header className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl lg:text-5xl text-[#0d3874] mb-6 tracking-tight">
            {products.length > 0 ? products[0].category_name : "Parking Management"} – CASS
          </h2>
