...
const Cochrane = () => {
  ...
  useEffect(() => {
    const fetchCochraneProducts = async () => {
      try {
        const response = await fetch(`${IMAGE_BASE_URL}/api/products/brand/Cochrane`);
        const data = await response.json();
        setProducts(data);
        setLoading(false);
      } catch (error) {
        console.error("Error fetching Cochrane products:", error);
        setLoading(false);
      }
    };
    fetchCochraneProducts();
  }, []);

  if (loading) return <div className="min-h-screen flex items-center justify-center font-figtree text-[#0d3874]">Loading Cochrane...</div>;
