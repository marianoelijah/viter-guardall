...
const Acti = () => {
  ...
  useEffect(() => {
    const fetchActiProducts = async () => {
      try {
        const response = await fetch(`${IMAGE_BASE_URL}/api/products/brand/ACTi`);
        const data = await response.json();
        setProducts(data);
        setLoading(false);
      } catch (error) {
        console.error("Error fetching ACTi products:", error);
        setLoading(false);
      }
    };
    fetchActiProducts();
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-figtree text-[#0d3874]">Loading ACTi Catalog...</div>;
      ...
      <div className="max-w-7xl mx-auto">
        <header className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl lg:text-5xl text-[#0d3874] mb-6 tracking-tight">
            {products.length > 0 ? products[0].category_name : "CCTVs"} - ACTi
          </h2>
