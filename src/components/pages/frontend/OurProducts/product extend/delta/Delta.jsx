...
const Delta = () => {
  ...
  useEffect(() => {
    const fetchDeltaProducts = async () => {
      try {
        const response = await fetch(`${IMAGE_BASE_URL}/api/products/brand/Delta`);
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-figtree text-[#0d3874]">Loading Delta Scientific Systems...</div>;
      ...
      <div className="max-w-7xl mx-auto">
        <header className="text-center mb-16">
          <h2 className="text-lg md:text-5xl lg:text-5xl text-[#0d3874] mb-6 tracking-tight">
            {products.length > 0 ? products[0].category_name : "Vehicle Barrier Systems"} – Delta
          </h2>
