...
const ProductCard = ({ title, description, image, detailRoute }) => (
  <div className="bg-white border border-gray-200 rounded-xl p-5 flex flex-col h-full transition-all duration-300 ease-in-out hover:-translate-y-2 hover:shadow-xl cursor-pointer group">
    ...
    <div className="flex-grow">
      <h3 className="text-[#1a365d] font-semibold text-xl uppercase mb-3 leading-tight tracking-wide">
        {title}
      </h3>
      ...
      <span className='block mt-12 text-gray-500 text-sm tracking-widest'>
        <NavLink to={detailRoute} className="text-[#184c9b] hover:underline mt-2 block">
          <h3 className='text-xl font-poppins hover:text-[#ff5f31] transition-colors duration-300'>View Details</h3>
        </NavLink>
      </span>
    </div>
  </div>
...
const Fermax = () => {
  ...
  useEffect(() => {
    const fetchFermaxProducts = async () => {
      try {
        const response = await fetch(`${IMAGE_BASE_URL}/api/products/brand/Fermax`);
        const data = await response.json();
        setProducts(data);
        setLoading(false);
      } catch (error) {
        console.error("Error fetching Fermax products:", error);
        setLoading(false);
      }
    };
    fetchFermaxProducts();
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-figtree">Loading Fermax...</div>;

  return (
    <div className="bg-[#e9eff6] min-h-screen py-16 px-6 lg:px-12 font-figtree">
      <div className="max-w-7xl mx-auto">
        <header className="text-center mb-16">
          <h2 className="text-4xl md:text-6xl lg:text-5xl text-[#0d3874] mb-6 tracking-tight">
            {products.length > 0 ? products[0].category_name : "Intercom & PA-BGM"} - Fermax
          </h2>
